...
class Pipeline(object):
    ...
    def updatePipeline(self):
        ...
        if self.wallx < -80:
            ...
            score += 1
            self.wallx = 400
            self.up_wally = random.randint(-200, 100)
            self.down_wally = random.randint(300, 600)
...
def createMap():
    ...
    # 显示管道
    screen.blit(Pipeline.pineUp, (Pipeline.wallx, Pipeline.up_wally))   # 上管道坐标位置
    screen.blit(Pipeline.pineDown, (Pipeline.wallx, Pipeline.down_wally))  # 下管道坐标位置
...
def checkDead():
    # 上方管子的矩形位置
    upRect = pygame.Rect(Pipeline.wallx, Pipeline.up_wally,
                         Pipeline.pineUp.get_width() - 10,
                         Pipeline.pineUp.get_height())

    # 下方管子的矩形位置
    downRect = pygame.Rect(Pipeline.wallx, Pipeline.down_wally,
...
if __name__ == '__main__':
    ...
    while True:
        ...
        up_location, down_location = 0, 500
        if start:
